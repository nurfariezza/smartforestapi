    ...
    def post(self,data=auth):
        try:
            ...
            if user['status'] != 1: 
                ...
                # email = session['user']
                # defaultsite= User().get_default_site(email)
                # list = user, defaultsite
                # print(defaultsite)
                # ctx ={
                #     "user": list
...
class UserListw(Resource):
    def get(self):
        ...
        try:
            q = """
                SELECT user.email,user.username, user.contactno, user.is_approval, user_cate.group_id, user_cate.group, role.role_id, role.role_name FROM user, role, user_cate WHERE role.role_id =user.roleid and user_cate.group_id = user.group_id
...
class CreateUserw(Resource):
    def post(self):
        ...
        try:
            ...
            # validate input
            is_validated = validate_user_form(data.get('username'),data.get('email'), data.get('password'),data.get('contactno'),data.get('is_approval'),data.get('roleid'),data.get('group_id'))
            if is_validated is not True:
                return dict(status='0', data=None, error=is_validated), 400
...
# api.add_resource(Profilew, '/w/profile/')

#both 
api.add_resource(RoleList, '/w/role/')
