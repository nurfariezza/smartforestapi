...
class Forest:
    ...
    def update_user(self, userid, email,username,password,contactno,siteid,is_approval,group_id,is_admin,access_role,created_by):
        ...
        try:
            ...
            if u_id is not None:
                query = "UPDATE user SET username =%s,password =%s, contactno=%s, is_approval=%s, group_id=%s, is_admin=%s, created_by=%s where u_id=%s"
                data =(username,password,contactno,is_approval,group_id,is_admin, created_by,u_id)
                cur.execute(query,data)
                con.commit()
    ...
    def get_last_koordinate(self,kompatmenid):
        try:
            ...
            # print(id_stesens)
            query = "SELECT line_to as stesen FROM koordinate where id=%s and kompatmen_id=%s"
    ...
    def bancian_selepas_tebangan(self,kompatment_id,no_pokok,no_tag,dbh,spesies,pepanjat,biltual):
        try:
            ...
            if (kompatment_id,no_pokok,no_tag,dbh,spesies,pepanjat,biltual) is not None:
                ...
                self.insert_log_aktiviti_pembalakan(pokok_id=no_pokok, aktiviti_id=5, start_date=None, end_date=None, bil_tual=biltual, isipadu_ha=None, litupan_pepanjat=pepanjat, status_keputusan=None, hidup_mati=None, jenis_kerosakan=None, tahap_kerosakan=None, diameter=None, request_by=None, request_date=None, approve_by=None, approve_date=None, rejected_by=None, rejected_date=None, dbh=dbh, no_petak=None)
                con.commit()
    ...
    def get_pembalak_pengeluaran(self, nfc_tag):
        try:
            ...
            s.scientific_name,p.no_pokok, p.dbh, p.koordinate_x, p.koordinate_y, p.isipadu_kasar, p.status_keputusan,p.status_keputusan as bolehditebang, 'tual_detail'
            from pokok p 
            LEFT JOIN spesies s ON  s.id=p.spesies_id
    ...
    def insert_log_aktiviti_pembalakan(self,pokok_id, aktiviti_id, start_date, end_date, bil_tual, isipadu_ha, litupan_pepanjat, status_keputusan, hidup_mati, jenis_kerosakan, tahap_kerosakan, diameter, request_by, 
    ...
    def get_senaraipokoktebang_detaillist_dashboard(self, kompatmenid):
        try:
            ...
            query = """select  p.kompatmen_id, p.no_tag as penandaan, p.no_pokok,p.spesies_id, s.scientific_name,
             p.dbh, p.bil_tual, p.isipadu_kasar from pokok p LEFT JOIN spesies s ON s.id=p.spesies_id where 
            p.kompatmen_id =%s and current_process_id=1 group by p.spesies_id"""
    ...
    def sendmail(self, username,email):
        try:
            message = """Hi {0}, <br><br>
                
            You have new request.<br>

            To view details, please login <a href="https://frim.redtone.com/"> here</a><br><br>

            This email is automatically generated by system. Please do not reply.""".format(username)
    ...
    def get_all_list(self):
        try:
            ...
            query = """select bil_tual as tual_lama, count(t.is_new_tual) as tual_tambahan, p.kompatmen_id, p.bil_tual, sl2.name as state,sl.name as district,k.hutan_id,hs.name as hutan_name, 
            ...
            t.status,ls.name,'aktiviti_name', t.is_new_tual, t.is_pengeluaran from pokok p LEFT JOIN spesies s ON s.id=p.spesies_id LEFT JOIN kompatmen k ON 
            ...
            RIGHT JOIN status_lookup ls ON t.status=ls.id where t.status in (1,2,3) GROUP BY p.no_pokok"""
    ...
    def get_approved_list(self):
        try:
            ...
            query = """select bil_tual as tual_lama, count(t.is_new_tual) as tual_tambahan, p.kompatmen_id, p.bil_tual, sl2.name as state,sl.name as district,
            k.hutan_id,hs.name as hutan_name, p.spesies_id, s.scientific_name,p.no_pokok, p.dbh, p.status_keputusan, t.notify_pegawai, 
            t.remark, u.username as request_by, t.request_date, t.status,ls.name from pokok p LEFT JOIN spesies s ON s.id=p.spesies_id 
    ...
    def get_reject_list(self):
        try:
            ...
            query = """select bil_tual as tual_lama, count(t.is_new_tual) as tual_tambahan, p.kompatmen_id, p.bil_tual, 
            sl2.name as state,sl.name as district,k.hutan_id,hs.name as hutan_name, p.spesies_id, s.scientific_name,p.no_pokok, 
            p.dbh, p.status_keputusan, t.notify_pegawai, t.remark, u.username as request_by, t.request_date, t.status,ls.name 
    ...
    def calculate_tual_price(self, no_pokok,nfc_tag,no_tual,panjang, dbh):
        try:
            ...
            data =(calc_tual_price,price_id, datetime.now(),nfc_tag, 1,no_pokok,no_tual)
            cur.execute(query,data)
            con.commit()
            return pricedetail
    ...
    def get_pegawai_pengeluaranSebenar_cukai(self, nfc_tag):
        try:
            ...
            s.scientific_name,p.no_pokok, p.dbh as dbh_pokok, p.koordinate_x, p.koordinate_y,t.no_tual, t.panjang_tual, t.diameter, t.harga_cukai
            from pokok p 
            LEFT JOIN spesies s ON  s.id=p.spesies_id
            LEFT JOIN kompatmen k ON p.kompatmen_id=k.persempadanan_id
            LEFT JOIN hutan_simpan hs ON hs.id=k.hutan_id
    ...
    def pegawai_confirm_tual_price(self, nfctag,pokok_id,spesies_id,diameter,tual_price,status,session_user):
          
        try:

            con, cur = initdb()
            #check if spesies same 
            query = """ select no_pokok, spesies_id from pokok where no_pokok=%s"""
            data =(pokok_id,)
            cur.execute(query,data)
            pdetail = [dict((cur.description[i][0], value) for i, value in enumerate(row)) for row in cur.fetchall()]

            for k in pdetail:
                current_spesies = k['spesies_id']


            if current_spesies!=spesies_id:
                query = """select s.scientific_name,sp.price, sp.id as price_id from spesies s
                LEFT JOIN spesies_price sp ON s.id=sp.spesies_id  
                where sp.spesies_id=%s"""
                ...
                tual_price= float(diameter)*float(spesies_price)



            query = """UPDATE tual SET notify_pegawai=1, status=2, harga_cukai=%s,request_by=%s where no_pokok=%s and nfcid=%s"""
            data =(tual_price,session_user,pokok_id,nfctag)
            # print(data)
            cur.execute(query,data)
            con.commit()
