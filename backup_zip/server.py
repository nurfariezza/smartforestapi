...
class PenandaanSempKompartmen(Resource):
    def post(self):
        ...
        try:
            ...
            is_validated = validate_penandaan_semp_form(data.get('hutansimpan_id'),data.get('kompatmen'), data.get('pokokstesen'),data.get('spesies'),data.get('koordinate_x'),data.get('koordinate_y'))
            # print(data)
            if is_validated is not True:
                return dict(message='Invalid data', data=None, error=is_validated), 400
...
class PenandaanPokok(Resource):
    def post(self):
        ...
        try:
            ...
            is_validated = validate_penandaanpokok(data.get('kompatmen_id'),data.get('no_pokok'),data.get('no_tag'),data.get('spesies_id'),data.get('dbh'),data.get('koordinate_x'),data.get('koordinate_y'),data.get('bil_tual'))
            if is_validated is not True:
                return dict(message='Invalid data', data=None, error=is_validated), 400
...
class Update_PenandaanPokok(Resource):
    def post(self,nfctag):
        ...
        try:
            ...
            is_validated = validate_update_penandaanpokok(data.get('kompatmen_id'),data.get('spesies_id'),data.get('dbh'),data.get('koordinate_x'),data.get('koordinate_y'),data.get('bil_tual'),data.get('no_pokok'))
            if is_validated is not True:
                return dict(message='Invalid data', data=None, error=is_validated), 400
...
class BancianSelepasTebangan(Resource):
    def post(self,nfctag):
        ...
        try:
            ...
            is_validated = validate_bancian_selepas_tebangan(data.get('kompatmen_id'),data.get('no_pokok'),data.get('no_tag'),data.get('dbh'),data.get('spesies'),data.get('pepanjat'),data.get('bil_tual'))
            if is_validated is not True:
                return dict(message='Invalid data', data=None, error=is_validated), 400
...
class Pembalak_Noti(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            is_validated = validate_body_value(data.get('body'))
            ...
            serverToken = 'AAAAjhnYBjI:APA91bFkRFa9PCXLJ86gSGNnBg73fN_xlIp19KTIeZkx2cdAXxVW3VOwx4KGbfqP-JFu_BPGpNrtXsUWJme8AyfNe4ze7F_g1Sz7Sdq_bVbPvWRBTH0sjWqjLHWsjo4mXTdVwidq6F0I'
            headers = {
            ...
            body = {
                    'notification': {'title': 'FRIM Notification',
                                        'body': bodydetail,
                                        'sound': 'default'
                                        },
                    'to':'/topics/FRIMAPI',
...
api.add_resource(Pembalak_Noti, '/notification/')
