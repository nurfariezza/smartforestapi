...
def validate_user_form(username, email,groupid, is_approval, siteid):

    if not (username and email and groupid and is_approval and siteid):
        return {
...
def validate_penandaan_semp_form(state, district,name, keluasan, coor_x, coor_y,jarak, bearing):

    if not (state and district and name and  keluasan and coor_x and coor_y and jarak and bearing):
        return {
            'keluasan': 'Keluasan is required',
            'coor_x': 'Coordinate X is required',
            'coor_y': 'Coordinate Y is required',
            'jarak': 'Jarak  is required',
